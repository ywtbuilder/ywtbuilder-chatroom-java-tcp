...
public class thread extends Thread implements ActionListener {
    ...
    public thread(ServerSocket serverInter, JTextArea in, JTextArea out, JButton send_button) {
    ...
    public void actionPerformed(ActionEvent e) { // 输出公告
        try {
            ...
            for (int i = 0; i < sockets.index_serverOut; i++) {
                sockets.outStreams[i].write(("Server：" + this.message_out + "\n").getBytes());
    ...
    public void run() {
        ...
        try {
            ...
            this.send_button.addActionListener(this); // 点击后链接 actionPerformed
